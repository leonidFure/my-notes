...
```
Функция `func` может преобразовывать объекты типа `T` в тип `R`.
...
После заполнения будет игнорировать входящие сообщения. Это решение учитывает ограниченность ресурсов и позволяет настраивать емкость очереди, очереди такого типа используют, когда важность сообщений низка.
#### Ограниченная очередь с блокировкой
Очереди с возможностью блокировки производителя.
Такой подход лишает систему преимуществ асинхронной обработки, а так же снижает эффективность использования ресурсов.
...
TODO от сюда нужно взять только самое необходимое (в теории еще и разбить по подразделам) остальное вынести в отдельные заметки в Области знаний